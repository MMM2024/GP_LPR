...
class LPR_SA_model(nn.Module):

    '''

    SA is the self atttention, add two self attention layers between the encoder and decoder.

    head and inner are parameters of SA, the head number and dim of FFN inner features

    '''

    def __init__(self, nc, nclass, imgW=96, imgH=32, K=8, isSeqModel=True, head=2, inner=256, isl2Norm=False):

        super(LPR_SA_model, self).__init__()
        ...
        # insert two self attention layers between the encoder and decoder
    ...
    def forward(self, input, isTsne=False, isVis=False, isSAVis=False, isCNN=False):
        ...
        if self.isSeqModel:
            ...
            seq_out = seq_out.permute(0, 2, 1).reshape(b, c, w, h).permute(0, 1, 3, 2)
            #seq_out = seq_out.permute(0, 2, 1).reshape(b, c, h, w)
